...
RipperClaw tracks the lineage of every loadout. When you fork or remix someone else's work, the ancestry chain is preserved and displayed.

## How It Works

When you apply a published loadout and later publish your modified version, RipperClaw automatically:

1. Records the parent loadout's event ID
...
| Tag | Purpose |
|-----|---------|
| `["e", parent_id, "", "fork"]` | Links to the parent loadout event |
| `["p", author_npub]` | Credits the original author |

Multiple levels of remixing create a chain: your loadout points to its parent, which points to its parent, and so on.

## Remix Badges

Loadout cards in the feed show remix status:

- **Remix count**: how many times this loadout has been forked
- **Fork indicator**: shows this loadout is based on someone else's work
...
Each ancestor is clickable. Navigate the tree to see how a loadout evolved through the community.
...
- "Upstream updates available" notifications when a parent loadout is updated

## Publishing a Remix

You don't need to do anything special. The app automatically detects when you're publishing a loadout that originated from the feed:

1. Apply a loadout from the feed
